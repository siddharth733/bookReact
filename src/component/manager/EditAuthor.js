...
function EditAuthor() {
  ...
  useEffect(() => {
    axios.get(`/api/edit-author/${category_id}`).then(res=>{
  ...
  const updateCategory = (e) => {
    ...
    axios.put(`/api/update-author/${category_id}`,data).then(res =>{
      if(res.data.status===200){
        swal('Success',res.data.message,"success");
        setError([])
        navigate('/manager/view-salemanager')
  ...
  return (
    <div className='cards'>
        <div className='card shadow w-75'>
            ...
            <h3 className='fw-bold'>Update Author</h3>
            </div>
